function update_input!(
    ...
    input_candidates = append!([dst(e) for e in edges(graph) if !(e in union(inneighbors(graph, agent_idx), agent_idx))], [1:(agent_idx - 1); (agent_idx + 1):nv(graph)])
    ...
    for neighbor in inneighbors(graph, agent_idx)
        push!(unfollow_candidates, (neighbor, abs(this_agent.opinion - agent_list[neighbor].opinion)))
    end
